...
public class WebSocketHandlerHelper {
    ...
    private static final String TAG = "WebSocketHandlerHelper";
    ...
    WebSocketHandlerHelper(Context mContext) {
...
//                .subscribe();
        ...
        Disposable onTextMessageDisposable = rxWebSocket.onTextMessage()
                ...
                .subscribe(socketMessageEvent -> {
                    ...
                    if (BuildConfig.DEBUG)
                        Log.i(TAG, "Message: ".concat(socketMessageEvent.getText()));
        ...
        Disposable onFailureDisposable = rxWebSocket.onFailure()
                ...
                .subscribe(socketFailureEvent -> {
                    Log.i(TAG, "onFailure: ".concat(socketFailureEvent.getException().getMessage()));
                    rxWebSocket.connect();
                }, Throwable::printStackTrace);
                ...
                if (objX.has("out")) {
                    ...
                    for (int j = 0; j < outArray.length(); j++) {
                        ...
                        if ((outObj.has("addr") && BIP47Meta.getInstance().getPCode4Addr(outObj.getString("addr")) != null) ||
                            ...
                            if (BuildConfig.DEBUG)
                                Log.i(TAG, "received from " + out_addr);
                            ...
                            if (pcode != null && idx > -1) {
                                ...
                                for (int i = idx; i < (idx + BIP47Meta.INCOMING_LOOKAHEAD); i++) {
                                    if (BuildConfig.DEBUG)
                                        Log.i(TAG, "receive from " + i + ":" + BIP47Util.getInstance(context).getReceivePubKey(new PaymentCode(pcode), i));
                            ...
                            if (BuildConfig.DEBUG)
                                Log.i(TAG, "addr:" + outObj.getString("addr"));
